...
interface DiningCreateContainerProps {
  ...
  onSubmit?: (data: DiningCreateFormValues) => void;
}
...
export function DiningCreateContainer({
  ...
  const onFormSubmit = async (data: DiningCreateFormValues) => {
    try {
      const requestBody = {
        diningDate: data.diningDateTime?.toISOString(),
        voteDueDate: data.voteDeadline?.toISOString(),
    ...
    >
      <div className="flex flex-1 flex-col">
        <div className="flex w-full flex-col gap-8">
          <DiningDateTimeField name="diningDateTime" control={control} />
          <DiningVoteDeadlineField name="voteDeadline" control={control} />
